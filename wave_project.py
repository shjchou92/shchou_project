...
def wave1d_ani(f_init):
	...
	ani = animation.FuncAnimation(fig, animate, string.iteration(50), 
	...
	try:
		...
		os.system('clear')  # clears the screen in terminal
...
def wave2d_ani(function):
	...
	wave = waves.Wave2D(f_init,c=1.5)
	...
	if func_name == 'wave_gaussian2d':
		...
		ani = animation.FuncAnimation(fig, update, wave.iteration(2), 
									  interval=1)
	...
	while True:
		...
		try:
			...
			if answer.lower()[0] == 'o' or answer[0] == '1':
				...
				print("Sorry, '{}' is not an option.".format(user))
		except:
